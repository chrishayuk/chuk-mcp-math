# chuk_mcp_functions/mcp_pydantic_base.py
import os
import json
import inspect
...
if PYDANTIC_AVAILABLE:
    ...
    class ValidationError(Exception):
        ...
        def __init__(self, message: str, field_path: str = "", error_type: str = "validation_error"):
    ...
    def _deep_validate(name: str, value: Any, expected: Any, path: str = "") -> Any:
        ...
        if value is None:
            if _is_optional(expected):
                return None
            raise ValidationError(f"field required", current_path, "missing")
        ...
        if origin in (list, List):
            if not isinstance(value, list):
                raise ValidationError(f"value is not a valid list", current_path, "type_error")
        ...
        if origin in (dict, Dict):
            if not isinstance(value, dict):
                raise ValidationError(f"value is not a valid dict", current_path, "type_error")
    ...
    class Field:
        ...
        __slots__ = ("default", "default_factory", "alias", "description", 
                    "title", "required", "kwargs", "json_schema_extra")
    ...
    class McpPydanticBase:
        ...
        def _validate_required_fields(self, values: Dict[str, Any]):
            ...
            if missing:
                raise ValidationError(f"Missing required fields: {', '.join(missing)}", error_type="missing_fields")
            ...
            for name, expected_type in hints.items():
                ...
                if name in values:
                    try:
                        ...
                        if e.error_type in ("missing_fields", "missing"):
                            raise
        ...
        def model_validate(cls, data: Union[Dict[str, Any], Any]):
            ...
            if isinstance(data, dict):
                ...
                raise ValidationError(f"Cannot validate {type(data)} as {cls.__name__}", error_type="invalid_input")
