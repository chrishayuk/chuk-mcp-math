#!/usr/bin/env python3
# chuk_mcp_functions/math/trigonometry/hyperbolic.py
"""
...
@mcp_function(
    ...
    examples=[
        ...
        {"input": {"x": 1}, "output": 1.1752011936438014, "description": "sinh(1) ≈ 1.175"},
        {"input": {"x": -1}, "output": -1.1752011936438014, "description": "sinh(-1) ≈ -1.175 (odd function)"},
        {"input": {"x": 0.5}, "output": 0.5210953054937474, "description": "sinh(0.5) ≈ 0.521"}
    ]
)
...
@mcp_function(
    ...
    examples=[
        ...
        {"input": {"x": 1}, "output": 1.5430806348152437, "description": "cosh(1) ≈ 1.543"},
        {"input": {"x": -1}, "output": 1.5430806348152437, "description": "cosh(-1) = cosh(1) (even function)"},
        {"input": {"x": 0.5}, "output": 1.1276259652063807, "description": "cosh(0.5) ≈ 1.128"}
    ]
)
...
@mcp_function(
    ...
    examples=[
        ...
        {"input": {"x": 1}, "output": 0.7615941559557649, "description": "tanh(1) ≈ 0.762"},
        {"input": {"x": -1}, "output": -0.7615941559557649, "description": "tanh(-1) = -tanh(1) (odd function)"},
        {"input": {"x": 10}, "output": 0.9999999958776927, "description": "tanh(10) ≈ 1 (saturates)"}
    ]
)
...
@mcp_function(
    ...
    cache_ttl_seconds=3600,
    examples=[
        {"input": {"x": 1}, "output": 0.8509181282393216, "description": "csch(1) ≈ 0.851"},
        {"input": {"x": -1}, "output": -0.8509181282393216, "description": "csch(-1) = -csch(1) (odd function)"},
        {"input": {"x": 0.5}, "output": 1.9190347513349437, "description": "csch(0.5) ≈ 1.919"}
    ]
)
...
@mcp_function(
    ...
    examples=[
        ...
        {"input": {"x": 1}, "output": 0.6480542736638855, "description": "sech(1) ≈ 0.648"},
        {"input": {"x": -1}, "output": 0.6480542736638855, "description": "sech(-1) = sech(1) (even function)"},
        {"input": {"x": 2}, "output": 0.26580222883407969, "description": "sech(2) ≈ 0.266"}
    ]
)
...
@mcp_function(
    ...
    cache_ttl_seconds=3600,
    examples=[
        {"input": {"x": 1}, "output": 1.3130352854993312, "description": "coth(1) ≈ 1.313"},
        {"input": {"x": -1}, "output": -1.3130352854993312, "description": "coth(-1) = -coth(1) (odd function)"},
        {"input": {"x": 0.1}, "output": 10.033331119703451, "description": "coth(0.1) ≈ 10.033 (large for small x)"}
    ]
)
...
@mcp_function(
    ...
    cache_ttl_seconds=3600,
    examples=[
        {"input": {"x": 1}, "output": {"sinh": 1.1752011936438014, "cosh": 1.5430806348152437, "tanh": 0.7615941559557649, "csch": 0.8509181282393216, "sech": 0.6480542736638855, "coth": 1.3130352854993312}, "description": "All hyperbolic functions at x=1"},
        {"input": {"x": 0}, "output": {"sinh": 0.0, "cosh": 1.0, "tanh": 0.0, "csch": None, "sech": 1.0, "coth": None}, "description": "Functions at x=0 (some undefined)"}
    ]
)
...
@mcp_function(
    ...
    cache_strategy="memory",
    examples=[
        {"input": {"x": 1}, "output": {"identity_holds": True, "calculated_value": 1.0, "error": 0.0}, "description": "Identity verification at x=1"},
        {"input": {"x": 5}, "output": {"identity_holds": True, "calculated_value": 1.0000000000000009, "error": 8.881784197001252e-16}, "description": "Identity with small numerical error"}
    ]
)
...
@mcp_function(
    ...
    cache_strategy="memory",
    examples=[
        {"input": {"a": 1, "x": 0}, "output": {"y": 1.0, "slope": 0.0, "arc_length": 0.0, "curve_type": "catenary"}, "description": "Catenary at lowest point"},
        {"input": {"a": 2, "x": 1}, "output": {"y": 2.5430806348152437, "slope": 1.1752011936438014, "arc_length": 2.3504023872876028, "curve_type": "catenary"}, "description": "Catenary curve properties"}
    ]
)
...
__all__ = [
    ...
    # Utility functions
    'hyperbolic_functions', 'verify_hyperbolic_identity', 'catenary_curve'
]
...
if __name__ == "__main__":
    ...
    async def test_hyperbolic_functions():
        ...
        for x in test_values:
            ...
            cosh_x = await cosh(x)
            tanh_x = await tanh(x)
            print(f"  x = {x:4.1f}: sinh = {sinh_x:8.4f}, cosh = {cosh_x:8.4f}, tanh = {tanh_x:8.4f}")
        ...
        for x in test_values_nonzero:
            try:
                ...
                sech_x = await sech(x)
                coth_x = await coth(x)
                print(f"  x = {x:4.1f}: csch = {csch_x:8.4f}, sech = {sech_x:8.4f}, coth = {coth_x:8.4f}")
        ...
        for a, x in catenary_cases:
            ...
            print(f"  Catenary a={a}, x={x}: y={y:.3f}, slope={slope:.3f}, arc_length={arc_len:.3f}")
