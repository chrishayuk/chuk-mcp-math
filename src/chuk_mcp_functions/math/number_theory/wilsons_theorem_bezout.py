#!/usr/bin/env python3
# chuk_mcp_functions/math/number_theory/wilsons_theorem_bezout.py
"""
...
@mcp_function(
    ...
    examples=[
        ...
        {"input": {"n": 4}, "output": False, "description": "4 is composite, 3! ≢ -1 (mod 4)"}
    ]
)
...
@mcp_function(
    ...
    examples=[
        {"input": {"p": 7}, "output": {"factorial": 720, "factorial_mod_p": 6, "is_minus_one": True, "verification": "6! = 720 ≡ 6 ≡ -1 (mod 7)"}, "description": "Wilson's theorem verification for 7"},
        {"input": {"p": 5}, "output": {"factorial": 24, "factorial_mod_p": 4, "is_minus_one": True, "verification": "4! = 24 ≡ 4 ≡ -1 (mod 5)"}, "description": "Wilson's theorem verification for 5"},
        {"input": {"p": 11}, "output": {"factorial_mod_p": 10, "is_minus_one": True}, "description": "Wilson's theorem verification for 11"},
        {"input": {"p": 13}, "output": {"factorial_mod_p": 12, "is_minus_one": True}, "description": "Wilson's theorem verification for 13"}
    ]
)
...
@mcp_function(
    ...
    estimated_cpu_usage="high",
    examples=[
        {"input": {"n": 17}, "output": True, "description": "17 is prime (optimized test)"},
        {"input": {"n": 15}, "output": False, "description": "15 is composite (optimized test)"},
        {"input": {"n": 19}, "output": True, "description": "19 is prime (optimized test)"},
        {"input": {"n": 21}, "output": False, "description": "21 is composite (optimized test)"}
    ]
)
...
@mcp_function(
    ...
    examples=[
        {"input": {"a": 30, "b": 18}, "output": {"gcd": 6, "x": -1, "y": 2, "verification": "30*(-1) + 18*2 = 6"}, "description": "Bézout coefficients for 30, 18"},
        {"input": {"a": 17, "b": 13}, "output": {"gcd": 1, "x": -3, "y": 4, "verification": "17*(-3) + 13*4 = 1"}, "description": "Bézout coefficients for coprime numbers"},
        {"input": {"a": 48, "b": 18}, "output": {"gcd": 6, "x": 1, "y": -2, "verification": "48*1 + 18*(-2) = 6"}, "description": "Another Bézout example"},
        {"input": {"a": 25, "b": 15}, "output": {"gcd": 5, "x": -1, "y": 2, "verification": "25*(-1) + 15*2 = 5"}, "description": "Bézout for 25, 15"}
    ]
)
...
@mcp_function(
    ...
    execution_modes=["local", "remote"],
    examples=[
        {"input": {"a": 24, "b": 16}, "output": {"gcd": 8, "bezout_coeffs": [1, -1], "all_solutions": "x = 1 + 2t, y = -1 - 3t"}, "description": "Extended GCD with general solution"},
        {"input": {"a": 35, "b": 21}, "output": {"gcd": 7, "bezout_coeffs": [-2, 3], "verification_multiple": True}, "description": "GCD with verification"},
        {"input": {"a": 100, "b": 45}, "output": {"gcd": 5, "alternative_solutions": [[-4, 9], [5, -11]]}, "description": "Multiple Bézout representations"},
        {"input": {"a": 13, "b": 8}, "output": {"gcd": 1, "bezout_coeffs": [-3, 5]}, "description": "Coprime numbers"}
    ]
)
async def extended_gcd_bezout(a: int, b: int, find_alternatives: bool = True) -> Dict:
    ...
    if gcd_val > 0:
        ...
        if gcd_val == 1:
            result["applications"] = {
                "modular_inverse": f"{x0} is the multiplicative inverse of {a} modulo {b}" if b > 1 and x0 % b > 0 else None,
...
@mcp_function(
    ...
    execution_modes=["local", "remote"],
    examples=[
        {"input": {"a": 17, "b": 13, "applications": ["modular_inverse", "diophantine"]}, "output": {"modular_inverse_17_mod_13": 9, "diophantine_solvable": True}, "description": "Applications for coprime numbers"},
        {"input": {"a": 30, "b": 42, "applications": ["gcd", "lcm"]}, "output": {"gcd": 6, "lcm": 210, "simplification": "30/42 = 5/7"}, "description": "GCD/LCM applications"},
        {"input": {"a": 25, "b": 15, "applications": ["fraction_reduction"]}, "output": {"reduced_fraction": "5/3", "common_factor": 5}, "description": "Fraction reduction"},
        {"input": {"a": 11, "b": 7, "applications": ["chinese_remainder"]}, "output": {"coprime": True, "crt_applicable": True}, "description": "Chinese Remainder Theorem applicability"}
    ]
)
async def bezout_applications(a: int, b: int, applications: List[str] = None) -> Dict:
    """
    ...
    for app in applications:
        if app == "modular_inverse":
            ...
            if gcd_val == 1 and b > 1:
                ...
                results["modular_inverse"] = {"error": f"No modular inverse exists (gcd = {gcd_val} ≠ 1)"}
            ...
            if b != 0:
                ...
                results["fraction_reduction"] = {"error": "Cannot reduce fraction with denominator 0"}
            ...
            results["diophantine"] = {
                ...
                "general_solution": f"x = {x}(c/{gcd_val}) + {b//gcd_val}t, y = {y}(c/{gcd_val}) - {a//gcd_val}t" if gcd_val > 0 else "undefined",
                "examples": {
                    f"c = {gcd_val}": {"x": x, "y": y},
            ...
            results["chinese_remainder"] = {
                ...
                "explanation": "CRT applies when moduli are pairwise coprime" if gcd_val == 1 else f"CRT not applicable (gcd = {gcd_val} ≠ 1)"
            }
...
__all__ = [
    # Wilson's theorem
    'wilson_theorem_test', 'wilson_theorem_verify', 'optimized_wilson_test',
    
    # Bézout's identity
    'bezout_identity', 'extended_gcd_bezout', 'bezout_applications'
]
...
async def test_wilson_bezout():
    ...
    for a, b, expected_gcd in bezout_test_cases:
        ...
        print(f"   Bézout({a:2d}, {b:2d}): gcd={gcd_val} {'✓' if gcd_correct else '✗'}, "
...
async def demo_wilson_theorem():
    ...
    for p in small_primes:
        ...
        if verification.get("factorial"):
            print(f"  p={p}: ({p-1})! = {verification['factorial']} ≡ {verification['factorial_mod_p']} ≡ -1 (mod {p}) ✓")
        else:
