...
try:
    ...
    from chuk_mcp_math.arithmetic import (
        get_arithmetic_functions,
        get_arithmetic_constants,
        validate_arithmetic_installation,
    )
    from chuk_mcp_math.arithmetic.sequences import harmonic_series
    from chuk_mcp_math.arithmetic.comparison import (
        sort_numbers,
...
class AsyncArithmeticDiagnostics:
    ...
    async def run_full_diagnostics(self) -> DiagnosticReport:
        ...
        self.log(
            f"Environment: Python {env_info['python_version']} on {env_info['platform']}"
        )
        ...
        return self._generate_report(
            env_info, function_counts, perf_stats, concurrency_stats
        )
    ...
    async def _discover_functions(self) -> Dict[str, int]:
        ...
        try:
            ...
            counts = {
                ...
                "arithmetic_functions": sum(
                    len(cat) for cat in arithmetic_functions.values()
                ),
                "basic_operations": len(
                    arithmetic_functions.get("basic_operations", {})
                ),
                ...
                "advanced_operations": len(
                    arithmetic_functions.get("advanced_operations", {})
                ),
    ...
    async def _test_basic_functionality(self):
        ...
        await self._test_category(
            ...
            [
                (
                    "arithmetic_sequence",
                    lambda: arithmetic_sequence(2, 3, 4),
                    [2, 5, 8, 11],
                ),
    ...
    async def _run_single_test(
        self, test_name: str, test_func: Any, expected: Any = None
    ):
        ...
        try:
            ...
            if expected is not None:
                ...
                if not success:
                    ...
                    self.results.append(
                        TestResult(test_name, False, duration, error_msg)
                    )
    ...
    async def _test_async_features(self):
        ...
        await self._run_single_test("async.basic_execution", lambda: add(1, 2))

        # Test parallel execution - ensure all functions are awaitable
        start_time = time.time()
        results = await asyncio.gather(
            add(1, 2), multiply(3, 4), is_prime(17), fibonacci(10)
        )
        ...
        self.log(
            f"    ✅ Parallel execution: {parallel_duration:.4f}s for 4 operations",
            "PERF",
        )
    ...
    async def _test_concurrency(self) -> Dict[str, Any]:
        ...
        self.log(
            f"    {concurrent_tasks} concurrent adds: {concurrent_duration:.4f}s "
            f"({concurrency_stats['concurrent_adds']['throughput']:.1f} ops/s)",
            "PERF",
        )
        ...
        self.log(
            f"    {len(mixed_tasks)} mixed operations: {mixed_duration:.4f}s", "PERF"
        )
    ...
    async def _test_caching(self):
        ...
        for i, op in enumerate(expensive_operations):
            ...
            if time2 < time1 * 0.1:  # Significant speedup
                self.log(
                    f"    ✅ Cache hit detected: {time1:.6f}s → {time2:.6f}s", "SUCCESS"
                )
    ...
    async def _test_error_handling(self):
        ...
        error_tests = [
            (
                ...
                lambda: divide(5, 0),
                ValueError,
            ),  # Fixed: Changed from ZeroDivisionError
        ...
        for test_name, test_func, expected_error in error_tests:
            try:
                ...
                if expected_error is None:
                    # No error expected
                    self.results.append(
                        TestResult(f"error_handling.{test_name}", True, 0.0)
                    )
                else:
    ...
    async def _stress_test_concurrency(self):
        ...
        self.log(
            f"    200 concurrent operations: {duration:.4f}s, "
            f"{successes}/{len(tasks)} successful",
            "PERF",
        )
    ...
    async def _stress_test_large_operations(self):
        ...
        large_ops = [
            ...
            (
                "large_sort",
                lambda: sort_numbers(list(range(5000, 0, -1))),
        ...
        for op_name, op_func in large_ops:
            ...
            try:
                ...
                duration = time.time() - start_time
                self.results.append(
                    TestResult(f"stress.{op_name}", False, duration, str(e))
                )
    ...
    async def _stress_test_memory_usage(self):
        ...
        try:
            ...
            self.results.append(TestResult("stress.memory_usage", False, 0.0, str(e)))
    ...
    def _generate_report(
        ...
        if len(failed_tests) > 0:
            recommendations.append(
                f"⚠️  {len(failed_tests)} tests failed - review error details"
            )
        ...
        if function_counts.get("async_native_functions", 0) > 0:
            recommendations.append(
                "🚀 Async native functions detected - optimal for concurrent use"
            )
    ...
    def print_report(self, report: DiagnosticReport):
        ...
        print("\n🖥️  ENVIRONMENT:")
        print(
            f"   Python: {report.environment.get('python_version', 'Unknown').split()[0]}"
        )
        print(f"   Platform: {report.environment.get('platform', 'Unknown')}")
        print(
            f"   Asyncio Policy: {report.environment.get('asyncio_policy', 'Unknown')}"
        )
...
async def main():
    ...
    parser.add_argument(
        "--quick", action="store_true", help="Run quick diagnostics only"
    )
