...
class TestPerfectNumberFunctions:
    ...
    async def test_is_perfect_number_known_values(self):
        ...
        for n in perfect_numbers:
            assert await is_perfect_number(n), f"{n} should be perfect"

        for n in non_perfect_numbers:
            assert not await is_perfect_number(n), f"{n} should not be perfect"
    ...
    async def test_is_abundant_number_known_values(self):
        ...
        for n in abundant_numbers:
            assert await is_abundant_number(n), f"{n} should be abundant"

        for n in non_abundant_numbers:
            assert not await is_abundant_number(n), f"{n} should not be abundant"
    ...
    async def test_is_deficient_number_known_values(self):
        ...
        for n in deficient_numbers:
            assert await is_deficient_number(n), f"{n} should be deficient"

        for n in non_deficient_numbers:
            assert not await is_deficient_number(n), f"{n} should not be deficient"
    ...
    async def test_perfect_numbers_up_to_small(self):
        ...
        assert result_100 == [6, 28], "Perfect numbers ≤ 100 should be [6, 28]"

        result_10 = await perfect_numbers_up_to(10)
        assert result_10 == [6], "Perfect numbers ≤ 10 should be [6]"

        result_5 = await perfect_numbers_up_to(5)
        assert result_5 == [], "Perfect numbers ≤ 5 should be []"

    @pytest.mark.asyncio
    async def test_perfect_number_functions_edge_cases(self):
        """Test edge cases for perfect number functions."""
        assert not await is_perfect_number(0)
        assert not await is_perfect_number(1)
        assert not await is_abundant_number(0)
        assert not await is_abundant_number(1)
        assert not await is_deficient_number(0)
        assert await is_deficient_number(1)  # 1 is deficient
...
class TestParametrized:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize(
        "n,expected_little,expected_big",
