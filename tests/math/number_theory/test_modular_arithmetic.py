    ...
    async def test_crt_solve_pairwise_coprime(self):
        ...
        test_cases = [
            ...
            (
                ...
                [7, 11, 13],
                (708, 1001),
            ),  # Was (895, 1001), corrected to (708, 1001)
        ]
    ...
    async def test_tonelli_shanks_algorithm(self):
        ...
        for a, p, expected in test_cases:
            ...
            assert sorted(result) == expected, (
                f"Square roots of {a} mod {p} should be {expected}, got {result}"
            )
    ...
    async def test_legendre_symbol_properties(self):
        ...
        for p in [7, 11, 13]:
            for a in range(20):
                ...
                assert legendre in [-1, 0, 1], "Legendre symbol should be in {-1,0,1}"
...
class TestPrimitiveRoots:
    ...
    async def test_all_primitive_roots_known_cases(self):
        ...
        expected_11 = [2, 6, 7, 8]
        assert roots_11 == expected_11, (
            f"All primitive roots mod 11 should be {expected_11}, got {roots_11}"
        )

        # All primitive roots mod 5: {2, 3}
        roots_5 = await all_primitive_roots(5)
        expected_5 = [2, 3]
        assert roots_5 == expected_5, (
            f"All primitive roots mod 5 should be {expected_5}, got {roots_5}"
        )
    ...
    async def test_primitive_root_verification(self):
        ...
        for p in primes_with_primitive_roots:
            ...
            if root is not None:
                ...
                assert order == p - 1, (
                    f"Primitive root {root} mod {p} should have order {p - 1}, got {order}"
                )
...
class TestDiscreteLogarithms:
    ...
    async def test_baby_step_giant_step_known_cases(self):
        ...
        for g, h, n, expected in test_cases:
            ...
            assert result == expected, (
                f"BSGS log_{g}({h}) mod {n} should be {expected}, got {result}"
            )
    ...
    async def test_baby_step_giant_step_vs_naive(self):
        ...
        for g, h, n in test_cases:
            ...
            assert naive_result == bsgs_result, (
                f"Naive and BSGS should agree for log_{g}({h}) mod {n}"
            )
        ...
        for p in primes:
            ...
            for a in range(2, p):
                if math.gcd(a, p) == 1:
                    ...
                    if order < p - 1:
                        assert a not in all_roots, (
                            f"{a} has order {order} < {p - 1}, so not primitive root"
                        )
...
class TestSpecialProperties:
    ...
    async def test_primitive_root_count(self):
        ...
        for p in primes:
            ...
            assert len(all_roots) == phi_p_minus_1, (
                f"Number of primitive roots mod {p} should be φ({p - 1}) = {phi_p_minus_1}"
            )
