...
class TestCatalanNumbers:
    ...
    async def test_is_catalan_number_known_catalan(self):
        ...
        for cat in known_catalan_numbers:
            assert await is_catalan_number(cat) == True, f"{cat} should be a Catalan number"
    ...
    async def test_is_catalan_number_non_catalan(self):
        ...
        for n in non_catalan:
            assert await is_catalan_number(n) == False, f"{n} should not be a Catalan number"
    
    @pytest.mark.asyncio
    async def test_is_catalan_number_edge_cases(self):
        """Test edge cases for Catalan number checking."""
        assert await is_catalan_number(0) == False  # 0 is not typically considered Catalan
    ...
    async def test_catalan_large_values(self):
        ...
        large_catalan = [
            (16, 35357670), (17, 129644790), (18, 477638700)
        ]
...
class TestStirlingNumbers:
    ...
    async def test_stirling_second_known_values(self):
        ...
        known_stirling_second = [
            (4, 2, 7), (5, 3, 25), (6, 3, 90), (5, 2, 15),
            (3, 2, 3), (4, 3, 6), (6, 4, 65), (7, 3, 301)
        ]
    ...
    async def test_narayana_symmetry(self):
        ...
        for n in range(2, 8):
            for k in range(1, n + 1):
                ...
                assert narayana_k == narayana_symmetric, f"N({n},{k}) should equal N({n},{n+1-k})"
...
class TestIntegrationAndProperties:
    ...
    async def test_growth_rates(self):
        ...
        for i in range(2, len(catalan_seq)):
            ...
            assert ratio > 1, f"Catalan numbers should be increasing"
        ...
        for i in range(2, len(bell_seq)):
            ...
            assert ratio > 1, f"Bell numbers should be increasing"
            if i > 3:
                assert ratio > 2, f"Bell numbers should grow rapidly"
