...
class TestSortNumbers:
    ...
    async def test_sort_negatives(self):
        ...
        assert await sort_numbers([-2, 0, 1, -5, 3], descending=True) == [3, 1, 0, -2, -5]
...
class TestRankNumbers:
    ...
    async def test_rank_large_list_async_yield(self):
        ...
        expected_max_rank = max(large_list) + 1  # Since we have values 0-99, max rank should be around 100
...
class TestPerformance:
    ...
    async def test_concurrent_execution(self):
        ...
        tasks = [
            sort_numbers([i, i-1, i+1]) for i in range(50)
        ]
...
class TestParametrized:
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("a,b,expected_min,expected_max", [
        (5, 3, 3, 5),
        (3, 5, 3, 5),
        (-2, 1, -2, 1),
        (0, 0, 0, 0),
    ...
    @pytest.mark.asyncio
    @pytest.mark.parametrize("numbers,expected_sorted,expected_min,expected_max", [
    ...
    async def test_list_operations_parametrized(self, numbers, expected_sorted, expected_min, expected_max):
