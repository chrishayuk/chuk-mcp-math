...
from chuk_mcp_functions.math.arithmetic.number_theory.iterative_sequences import (
    ...
    # Kaprekar sequences
    kaprekar_sequence, kaprekar_constant,
    
    # Happy numbers
    is_happy_number, happy_numbers,
    
    # Narcissistic numbers
    is_narcissistic_number, narcissistic_numbers,
...
class TestCollatzConjecture:
    ...
    async def test_collatz_sequence_properties(self):
        ...
        for n in test_numbers:
            ...
            assert sequence[-1] == 1, f"Collatz sequence should end with 1"
    ...
    async def test_collatz_stopping_time_known_values(self):
        ...
        for n, expected in test_cases:
            ...
            assert result == expected, f"collatz_stopping_time({n}) should be {expected}, got {result}"
...
class TestKaprekarSequences:
    ...
    async def test_kaprekar_sequence_known_cases(self):
        ...
        for n, digits, expected in test_cases:
            ...
            assert result == expected, f"kaprekar_sequence({n}, {digits}) should be {expected}, got {result}"
    ...
    async def test_kaprekar_sequence_properties(self):
        ...
        for n in test_numbers:
            ...
            if not self._is_repdigit(n, 4):
                assert 6174 in sequence or 0 in sequence, f"4-digit sequence should reach 6174 or 0"
    ...
    async def test_kaprekar_sequence_3_digit(self):
        ...
        for n in test_numbers:
            ...
            if not self._is_repdigit(n, 3):
                assert 495 in sequence or 0 in sequence, f"3-digit sequence should reach 495 or 0"
...
class TestHappyNumbers:
    ...
    async def test_is_happy_number_known_sad(self):
        ...
        known_sad = [2, 3, 4, 5, 6, 8, 9, 11, 12, 14, 15, 16, 17, 18, 20, 21, 22, 24, 25, 26, 27, 29, 30]
    ...
    async def test_happy_numbers_properties(self):
        ...
        for num in happy_list:
            assert await is_happy_number(num) == True, f"{num} in happy list should be happy"
...
class TestNarcissisticNumbers:
    ...
    async def test_narcissistic_numbers_generation(self):
        ...
        assert result_1000 == expected_1000, f"Narcissistic numbers up to 1000 should be {expected_1000}"
        
        # Up to 200
        expected_200 = [1, 2, 3, 4, 5, 6, 7, 8, 9, 153]
        result_200 = await narcissistic_numbers(200)
        assert result_200 == expected_200, f"Narcissistic numbers up to 200 should be {expected_200}"
    ...
    async def test_narcissistic_numbers_properties(self):
        ...
        for num in narcissistic_list:
            assert await is_narcissistic_number(num) == True, f"{num} should be narcissistic"
        
        # Should be in ascending order
        assert narcissistic_list == sorted(narcissistic_list), "Narcissistic numbers should be in ascending order"
...
class TestLookAndSaySequence:
    ...
    async def test_look_and_say_sequence_properties(self):
        ...
        for start in ["1", "2", "3", "11", "22"]:
            ...
            assert len(sequence) == 3, f"Should generate 3 terms"
    ...
    async def test_look_and_say_length_growth(self):
        ...
        for i in range(1, len(sequence)):
            ...
            assert len(sequence[i]) >= len(sequence[i-1]) - 2, f"Length shouldn't decrease dramatically"
    ...
    async def test_recaman_sequence_recurrence_verification(self):
        ...
        for i in range(1, len(sequence)):
            ...
            if candidate_subtract > 0 and candidate_subtract not in sequence[:i]:
                # Should subtract
                assert current == candidate_subtract, f"Should subtract at step {i}: {prev} - {i} = {candidate_subtract}"
            else:
                # Should add
...
class TestKeithNumbers:
    ...
    async def test_is_keith_number_known_keith(self):
        ...
        known_keith = [14, 19, 28, 47, 61, 75, 197, 742, 1104, 1537, 2208, 2580, 3684, 4788, 7385, 7647, 7909]
    ...
    async def test_is_keith_number_non_keith(self):
        ...
        non_keith = [10, 11, 12, 13, 15, 16, 17, 18, 20, 21, 22, 23, 24, 25, 26, 27, 29, 30]
    ...
    async def test_is_keith_number_edge_cases(self):
        ...
        for i in range(1, 10):
            assert await is_keith_number(i) == False, f"Single digit {i} should not be Keith"
...
class TestDigitalSequences:
    ...
    async def test_digital_sum_sequence_known_cases(self):
        ...
        for n, expected in test_cases:
            ...
            assert result == expected, f"digital_sum_sequence({n}) should be {expected}, got {result}"
    ...
    async def test_digital_sum_sequence_properties(self):
        ...
        for n in test_numbers:
            ...
            assert sequence[-1] < 10, f"Digital sum sequence should end with single digit"
            
            # Should be decreasing until single digit
            for i in range(len(sequence) - 1):
                assert sequence[i] >= sequence[i+1], f"Digital sum sequence should be non-increasing"
    ...
    async def test_digital_product_sequence_known_cases(self):
        ...
        test_cases = [
            ...
            (999, [999, 729, 126, 12, 2]), # 999 -> 9×9×9=729 -> 7×2×9=126 -> 1×2×6=12 -> 1×2=2
        ...
        for n, expected in test_cases:
            ...
            assert result == expected, f"digital_product_sequence({n}) should be {expected}, got {result}"
    ...
    async def test_digital_product_sequence_properties(self):
        ...
        for n in test_numbers:
            ...
            assert sequence[-1] < 10, f"Digital product sequence should end with single digit or 0"
            
            # If no zeros in digits, should generally decrease
            if '0' not in str(n):
                for i in range(len(sequence) - 1):
                    assert sequence[i] >= sequence[i+1], f"Digital product sequence should generally decrease"
    ...
    async def test_digital_product_sequence_zero_cases(self):
        ...
        for n in zero_cases:
            ...
            assert 0 in sequence, f"Digital product sequence with zeros should contain 0"
            assert sequence[-1] == 0, f"Digital product sequence with zeros should end with 0"
...
class TestIntegrationAndProperties:
    ...
    async def test_sequence_convergence_properties(self):
        ...
        for n in [123, 456, 999, 1234]:
            sequence = await digital_sum_sequence(n)
            assert sequence[-1] < 10, f"Digital sum sequence should reach single digit"
    ...
    async def test_sequence_length_relationships(self):
        ...
        for n in [3, 5, 7, 11]:
            sequence = await collatz_sequence(n)
            stopping_time = await collatz_stopping_time(n)
            assert len(sequence) - 1 == stopping_time, f"Length relationship failed for Collatz({n})"
        ...
        for i in range(1, len(sequence)):
            ...
            assert len(sequence[i]) >= len(sequence[i-1]) * 0.8, f"Look-and-say should generally grow"
    ...
    async def test_mathematical_invariants(self):
        ...
        recaman_seq = await recaman_sequence(20)  # Test with smaller sequence to avoid later duplicates
        assert len(recaman_seq) == len(set(recaman_seq)), "Recamán sequence should have no duplicates in early terms"
        ...
        for n in [153, 370, 371, 1634]:
            assert await is_narcissistic_number(n) == True, f"{n} should be narcissistic"
...
class TestPerformance:
    ...
    async def test_all_functions_are_async(self):
        ...
        assert results[0] == [7, 22, 11, 34, 17, 52, 26, 13, 40, 20, 10, 5, 16, 8, 4, 2, 1]  # collatz_sequence(7)
    ...
    async def test_memory_efficiency(self):
        ...
        assert all(seq[-1] == 1 for seq in collatz_sequences), "All Collatz sequences should end with 1"
