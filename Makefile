.PHONY: clean clean-pyc clean-build clean-test clean-all test test-cov coverage-report build publish publish-test publish-manual help install dev-install version bump-patch bump-minor bump-major release lint format typecheck security check run-demos run-diagnostics
...
help:
	...
	@echo ""
	@echo "Examples & Demos:"
	@echo "  run-demos      - Run all demo scripts"
	@echo "  run-diagnostics - Run diagnostic scripts"
	@echo ""
	@echo "Build & Release:"
...
clean-test:
	...
	@rm -rf .coverage_subprocess 2>/dev/null || true
...
dev-install:
	...
	@if command -v uv >/dev/null 2>&1; then \
		uv pip install -e ".[dev,pydantic,cli]"; \
	else \
		pip install -e ".[dev,pydantic,cli]"; \
	fi
...
coverage-report:
	...
	@echo "================"
	@if command -v uv >/dev/null 2>&1; then \
		uv run coverage report --omit="tests/*,diagnostics/*" || echo "No coverage data found. Run 'make test-cov' first."; \
	else \
		coverage report --omit="tests/*,diagnostics/*" || echo "No coverage data found. Run 'make test-cov' first."; \
	fi
...
test-cov coverage:
	...
	@if command -v uv >/dev/null 2>&1; then \
		...
		echo "=========================="; \
		uv run coverage report --omit="tests/*,diagnostics/*" | tail -5; \
		echo ""; \
		...
		echo "Coverage Summary:"; \
		echo "=========================="; \
		coverage report --omit="tests/*,diagnostics/*" | tail -5; \
		echo ""; \
		echo "HTML coverage report saved to: htmlcov/index.html"; \
		exit $$exit_code; \
	fi
...
run-demos:
	...
	@if command -v uv >/dev/null 2>&1; then \
		for demo in examples/demos/*.py; do \
			...
			echo "Running $$demo..."; \
			echo "=================="; \
			PYTHONPATH=src uv run python "$$demo" || true; \
		done; \
		for app in examples/applications/*.py; do \
			echo ""; \
			echo "Running $$app..."; \
			echo "=================="; \
			PYTHONPATH=src uv run python "$$app" || true; \
		done; \
	else \
		for demo in examples/demos/*.py; do \
			echo ""; \
			echo "Running $$demo..."; \
			echo "=================="; \
			PYTHONPATH=src python3 "$$demo" || true; \
		done; \
		for app in examples/applications/*.py; do \
			echo ""; \
			echo "Running $$app..."; \
			echo "=================="; \
			PYTHONPATH=src python3 "$$app" || true; \
		done; \
	fi
	@echo ""
...
run-diagnostics:
	...
	@if command -v uv >/dev/null 2>&1; then \
		for diag in diagnostics/*.py; do \
			echo ""; \
			echo "Running $$diag..."; \
			echo "=================="; \
			PYTHONPATH=src uv run python "$$diag" || true; \
		done; \
	else \
		for diag in diagnostics/*.py; do \
			echo ""; \
			echo "Running $$diag..."; \
			echo "=================="; \
			PYTHONPATH=src python3 "$$diag" || true; \
		done; \
	fi
	@echo ""
...
publish-test: build
	...
	echo ""; \
	echo "Install with:"; \
	echo "  pip install --index-url https://test.pypi.org/simple/ chuk-mcp-math==$$version"
...
publish-manual: build
	...
	echo "Install with: pip install chuk-mcp-math==$$version"
